...
def loadFast(problem_size):
    folders = [f"/LDSA/dataset/{a}/{b}/{c}/" 
...
def countArtistName(df):
    tic = time.perf_counter()
    df.groupBy('artist_name').count().show()    
    toc = time.perf_counter()
    print(f"It took {toc - tic:0.4f} seconds")
    return toc-tic
...
def countYear(df):
    tic = time.perf_counter()
    changedTypedf.groupBy("year").count().show()
    toc = time.perf_counter()
    print(f"It took {toc - tic:0.4f} seconds")
    return toc-tic
...
def strong_scaling(number_of_processors, problem_size):
    """
    ...
    with open('experiment.csv', 'w', newline='') as csvfile:
        ...
        for processors in number_of_processors:
            ...
            for i in range(3):
                times = []
                times.append(processors)
                print("load binary")
                tic = time.perf_counter()
                loaded = sc.binaryFiles("hdfs://192.168.2.110:9000/LDSA/subset/A/*/*")
                loaded.take(1)
                toc = time.perf_counter()
                print(f"It took {toc - tic:0.4f} seconds")
                ...
                print("map to arrays")
                tic = time.perf_counter()
                df_mapped = loaded.map(lambda x: read(x[1]))
                df_mapped.count()
                toc = time.perf_counter()
                print(f"It took {toc - tic:0.4f} seconds")
                times.append(toc-tic)
                ...
                columns = ['artist_name', 'title', 'artist_familiarity', 
                            ...
                            'tempo', 'time_signature', 
                            'time_signature_confidence', 'track_id', 'year']
                df_converted = df_mapped.toDF(columns)
                ...
                'DecimalType', 'DoubleType', 'FloatType', 'IntegerType', 
                'LongType', 'ShortType', 'StringType', 'TimestampType']
                changedTypedf = df_converted.withColumn("year", df["year"].cast("Integer"))\
                                    .withColumn("track_id", df["track_id"].cast("Integer"))\
                                    .withColumn("artist_id", df["artist_id"].cast("Integer"))\
                                    .withColumn("song_id", df["song_id"].cast("Integer"))\
                                    .withColumn("duration", df["duration"].cast("Float"))\
                ...
                print("count artists")
                tic = time.perf_counter()
                changedTypedf.groupBy('artist_name').count().show()    
                toc = time.perf_counter()
                print(f"It took {toc - tic:0.4f} seconds")
                times.append(toc-tic)
